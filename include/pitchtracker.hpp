...
std::vector<T> autocor(std::vector<T> &audio_buff){
  ...
  fft_backward = ffts_init_ld(N*2, FFTS_BACKWARD);
...
template <typename T>
T mpm(const vector<double> audio_buff, int sample_rate){
  std::vector<T> out_real = autocor(audio_buffer);
